...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    updateProductQuantity: function (state, action: PayloadAction<{id:string,count:number}>) {
      const {id,count} = action.payload;
      const product = state.products.find((item) => item.product.id === id);
      if(product){
        state.totalCartPrice -= product.price * product.count;
        product.count = count;
        state.totalCartPrice += product.price * product.count;
      }
    }
...
export const {setCartInfo,removeProduct,clearCart,updateProductQuantity} = cartSlice.actions
